...
class Socialize {
	...
	function pre_get_posts() {
		...
		if ( is_admin() && ! empty( $post ) ) {
			...
			// Source Sans Web Font (include only if a web font is not selected in Theme Options)
			if ( ! function_exists( 'sds_web_fonts' ) || empty( $sds_theme_options['web_font'] ) )
				add_editor_style( str_replace( ',', '%2C', $protocol . '://fonts.googleapis.com/css?family=Source+Sans+Pro:400,600,700,900,400italic' ) ); // Google WebFonts (Source Sans)
